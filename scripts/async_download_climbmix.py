...
async def download_all_shards(
    ...
    logger.info(f"Validation shards: 1")
        ...
        async def download_with_semaphore(shard_idx: int):
            async with semaphore:
                await download_shard(
                    session, shard_idx, output_path, repo_id, base_url,
                    shard_pattern, coordinator, total_shards, USE_HF_HUB
                )
...
def download_process(
    ...
    asyncio.run(download_all_shards(
        output_dir, num_train_shards, repo_id, base_url,
        shard_pattern, coordinator, max_concurrent
    ))
...
def start_download_and_training(
    ...
    download_proc = mp.Process(
        ...
        args=(output_dir, num_train_shards, repo_id, base_url, shard_pattern, coordinator, max_concurrent)
    )
    download_proc.start()
