...
from scripts.async_download_climbmix import DownloadCoordinator, download_process, wait_for_training_ready 
from goda.logger import logger


def training_process( train_script: str, train_args: list, coordinator: DownloadCoordinator, min_shards: int, use_torchrun: bool = False, torchrun_args: list | None = None) -> None:
...
def main():
    ...
    # Download arguments
    parser.add_argument("--data-dir", type=str, default="data/climbmix", help="Directory to save/load dataset")
    parser.add_argument("--num-train-shards", type=int, default=10, help="Total number of training shards to download")
    parser.add_argument("--min-shards", type=int, default=3, help="Minimum shards before starting training")
    parser.add_argument("--max-concurrent-downloads", type=int, default=3, help="Maximum concurrent downloads")
    parser.add_argument("--max-shards-to-wait", type=int, default=-1, help="Maximum shards to wait for in dataloader (-1 = wait indefinitely)")
    
    # Training arguments
    parser.add_argument("--train-script", type=str, required=True, help="Path to training script")
    parser.add_argument("--use-torchrun", action="store_true", help="Use torchrun instead of python")
    parser.add_argument("--torchrun-args", type=str, default="", help="Arguments for torchrun (e.g., '--nproc_per_node=2')")
    parser.add_argument("--train-args", type=str, default="", help="Arguments for training script")
    
    parser.add_argument("--skip-download", action="store_true", help="Skip download, assume shards exist")
    ...
    train_proc = mp.Process(
        ...
        args=(args.train_script, train_args, coordinator, args.min_shards, 
              args.use_torchrun, torchrun_args)
    )
    train_proc.start()
