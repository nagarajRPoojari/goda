...
class GroupedQueryAttention(nn.Module):
    def __init__(self, cfg: Config, window_size: int = -1):
        ...
        self.wq = nn.Linear(
            cfg.embed_dim, self.n_heads * self.head_dim, bias=False, dtype=cfg.dtype
        )
        self.wk = nn.Linear(
            cfg.embed_dim, self.n_kv_heads * self.head_dim, bias=False, dtype=cfg.dtype
        )
        self.wv = nn.Linear(
            cfg.embed_dim, self.n_kv_heads * self.head_dim, bias=False, dtype=cfg.dtype
        )
        self.wo = nn.Linear(
            self.n_heads * self.head_dim, cfg.embed_dim, bias=False, dtype=cfg.dtype
        )
    ...
    def forward(self, x, kv_cache: KVCache | None = None, start_pos: int = 0):
    ...
    def _precompute_rotary_embeddings(
        self, seq_len, head_dim, base=100000, device=None, dtype: torch.dtype = torch.float32
    ):
...
class Gemma(nn.Module):
    ...
    def forward(
        self, input_ids: torch.Tensor, kv_caches: list[KVCache] | None = None, start_pos: int = 0
    ) -> torch.Tensor:
