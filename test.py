...
def _attn_backward_dK_dV(
    ...
    for q_i in range(SEQ_LEN // BLOCK_SIZE_Q):
        ...
        # Compute in bf16, accumulate in float32
        block_dV_delta = tl.dot(block_Pt.to(block_dO.dtype), block_dO)
        block_dV = (block_dV + block_dV_delta.to(tl.float32)).to(tl.float32)
        
        # dP = dO * V^T
        # dP^T = V * dO^T
        block_dPt = tl.dot(block_V, tl.trans(block_dO))
        ...
        # dK <= dK + dS^T * Q
        # Compute in bf16, accumulate in float32
        block_dK_delta = tau * tl.dot(block_dSt.to(block_Qt.dtype), tl.trans(block_Qt))
        block_dK = (block_dK + block_dK_delta.to(tl.float32)).to(tl.float32)
...
def _attn_backward_dQ(
    ...
    for kv_i in range(SEQ_LEN // BLOCK_SIZE_KV):
        ...
        # dP and dS.
        # Compute in bf16
        block_dP = tl.dot(block_dO, block_Vt)
        block_dS = block_P * (block_dP - Di[:, None])
        
        # note: We need to de-scale dq in the end, because kT was pre-scaled.
        # Compute in bf16, accumulate in float32
        block_dQ_delta = tau * tl.dot(block_dS.to(block_Kt.dtype), tl.trans(block_Kt))
        block_dQ = (block_dQ + block_dQ_delta.to(tl.float32)).to(tl.float32)
...
def _attn_forward_internal(
    ...
    for start_kv in range(lo, hi, BLOCK_SIZE_KV):
        ...
        # Compute PV in bf16, then convert to float32 for accumulation
        block_PV = tl.dot(block_P.to(block_V.dtype), block_V)
        block_O = (block_O + block_PV.to(tl.float32)).to(tl.float32)
